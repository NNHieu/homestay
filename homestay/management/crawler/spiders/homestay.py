...
class HomestayDotCom(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        image_urls = []
        image_titles = []
        for li in response.xpath('//ul[@id="lightgallery"]/li'):
            image_urls.append(li.xpath('@data-src').get())
            sub_html = li.xpath('@data-sub-html').get()
            image_titles.append(re.search(r'>([^><]+)</p>', sub_html).group(1))
        ...
        item = {'from': 'homestay.com', 'response': response, 'model': homestay, 'image_urls': image_urls, 'images': scrapy.Field(), 'image_titles': image_titles}
        return item
